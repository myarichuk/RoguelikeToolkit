...
namespace RoguelikeToolkit.Entities.Tests
{
	public class EntityInheritanceResolverTests
	{
		...
		public void Can_resolve_multilevel_inheritance()
		{
			...
			var templateWithInheritance = _inheritanceResolver.GetEffectiveTemplate(template);

			Assert.NotNull(templateWithInheritance); //sanity check

			Assert.Collection(templateWithInheritance.Tags.OrderBy(x => x),
				tag => Assert.Equal("tag1", tag),
				tag => Assert.Equal("tag2", tag),
				tag => Assert.Equal("tag3", tag),
				tag => Assert.Equal("tag4", tag));

			Assert.Collection(templateWithInheritance.Components.OrderBy(x => x.Key),
				kvp =>
				{
